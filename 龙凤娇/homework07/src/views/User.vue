...
export default {
  ...
  methods: {
    getAllUsers(){
      axios.get("http://localhost:8089/AllUsers")
          .then(resp => {
            console.log(resp);
    ...
    getUserById(){
      axios.get(`http://localhost:8089/user/${this.queryId}`)
          .then(resp => {
            console.log(resp);
    ...
    updateUser(){
      axios.put("http://localhost:8089/user",{
        id: this.id,
        username: this.username,
        address: this.address
      }).then( resp => {
    ...
    deleteUser(){
      axios.delete("http://localhost:8089/user", {
        params: {
          id: this.deleteId
        }
      }).then(resp => {
